...
export default function SignUpPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Mock sign-up for demonstration
    signIn({ name, email });
    router.push("/");
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[var(--color-background)] to-[#ffffff]">
      <div className="glass-card w-full max-w-md flex flex-col gap-8 p-8">
        <div className="text-center">
          <h1 className="text-3xl font-serif font-bold text-[var(--color-text)] mb-2">Join Stayvillow</h1>
          <p className="text-[var(--color-text-light)]">Create your account for luxury vacation stays</p>
        </div>
        ...
        <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
          <div className="space-y-2">
            <label htmlFor="name" className="text-sm font-medium text-[var(--color-text-light)]">Full Name</label>
            <input 
              type="text" 
              id="name"
          ...
          <div className="space-y-2">
            ...
            <input 
              ...
              required 
            />
            <p className="text-xs text-[var(--color-text-light)]/70">Password must be at least 8 characters</p>
          </div>
        ...
        <div className="text-center text-sm text-[var(--color-text-light)]">
          Already have an account? <Link href="/signin" className="text-[var(--color-primary)] hover:underline font-medium">Sign In</Link>
        </div>
      </div>
    </div>
  );
